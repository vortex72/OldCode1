...
Function GetXMLNodeValue(objXMLDoc, NodeName, Instance)
  ' Get the node value for any node name
  ' NodeName must be fully qualified "/parent/child/etc/"
  ' Instance starts at 0
  On Error Resume Next
  ...
  Dim objNode
  Set objNode = objXMLDoc.documentElement.selectNodes(NodeName)
  GetXMLNodeValue = objNode.Item(Instance).text
  Set objNode = Nothing
...
Function SendXMLPacket_ReturnXMLString(sSendXMLPacket, sURL)
  ...
  '----------------------------------------
  ' Use ASPTear 1.5 to retrieve the packet
  '----------------------------------------
  'Dim objAspTear
  'Set objAspTear = Server.CreateObject("SOFTWING.AspTear")
  'objAspTear.ConnectionTimeout = 5000 ' 5 seconds
  'On Error Resume Next
  'SendXMLPacket_ReturnXMLString = objAspTear.Retrieve(sURL, Request_POST, sSendXMLPacket, eSynUserName, eSynPassword)
  ...
  '----------------------------------------
  ' Use MS XMLHTTP Object to retreive the packet  
  '----------------------------------------
  Dim objXMLHTTP
  Set objXMLHTTP = Server.CreateObject("WinHTTP.WinHTTPRequest.5.1")
  On Error Resume Next
  objXMLHTTP.open "POST", sURL
  objXMLHTTP.SetRequestHeader "Content-Type", "application/x-www-form-urlencoded;"
  objXMLHTTP.send GetXMLDoc(sSendXMLPacket)
  SendXMLPacket_ReturnXMLString = objXMLHTTP.ResponseText
  If Err.number > 0 Then
    SendXMLPacket_ReturnXMLString = Err.Description
  End If
  On Error Goto 0
